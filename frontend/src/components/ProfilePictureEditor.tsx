...
const ProfilePictureEditor: React.FC<ProfilePictureEditorProps> = ({
  ...
  useEffect(() => {
    ...
    // Get stored filename from localStorage or userData (PRIORITY)
    ...
    if (currentImage && !isDefault) {
      if (storedFileName) {
        ...
        // Only extract from URL if no stored filename exists
        if (currentImage.startsWith("data:image/")) {
          setCurrentFileName("profile-picture");
        } else {
          try {
            ...
            setCurrentFileName(cleanFileName || "profile-picture");
          } catch {
            setCurrentFileName("profile-picture");
          }
        }
    ...
    // Reset change flag and image file when modal opens
    if (isOpen) {
  ...
  const handleSave = async () => {
    try {
      ...
      // Determine what to send: only include image when it was changed
      const imagePayload =
        hasImageChanged && !isDefaultImage
          ? imageFile
          : hasImageChanged
          ? null
          : undefined;
